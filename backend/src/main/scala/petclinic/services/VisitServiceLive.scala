...
final case class VisitServiceLive(dataSource: DataSource, vetService: VetService) extends VisitService {
  ...
  /** `create` uses the `getAll` method defined in the VetService to retrieve a
    * list of all the vets in the database and randomly selects one. It then
    * uses the Visit model's `make` method to create a new Visit. The Visit is
  ...
  /** `delete` uses `filter` to find a Visit in the database with an ID that
    * matches the one provided and deletes it.
    *
    * Unit is returned to indicate that we are running this method for its side
    * effects, a deleted Visit gives us no information. This will either fail or
    * succeed.
    */
  ...
  /** `get` uses `filter` to find a Visit in the database with an ID that
  ...
  /** `getForPet` uses `filter` to find all Visits in the database with a PetId
    * that matches the one provided, returning all Visits associated with a
    * given Pet.
    */
  ...
  /** `getAll` uses `query` to find all entries in the database of type Visit
    * and returns them.
    */
  ...
  /** `update` uses `filter` to find a Visit in the database with an ID that
    * matches the one provided and updates it with the provided optional values.
    *

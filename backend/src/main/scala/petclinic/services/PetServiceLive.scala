...
final case class PetServiceLive(dataSource: DataSource) extends PetService {
  ...
  /** create uses the make method defined in the Pet model to create a new Pet.
    * The Pet is formatted into a query string, then inserted into the database
    * using provideEnvironment to provide the datasource to the effect returned
    * by run. The created Pet is returned.
    */
  ...
  /** delete uses the filter method to find a Pet in the database whose ID
    * matches the one provided and deletes it. Unit is returned to indicate that
    * we are running this method for its side effects, a deleted Pet gives us no
  ...
  /** get uses the filter method to find a Pet in the database whose ID matches
  ...
  /** getAll uses the query method to find all entries in the database of type
    * Pet and returns them.
    */
  override def getAll: Task[List[Pet]] =
    run(query[Pet].sortBy(_.birthdate))
      .provideEnvironment(ZEnvironment(dataSource))

  /** update uses the filter method to find a Pet in the database whose ID
